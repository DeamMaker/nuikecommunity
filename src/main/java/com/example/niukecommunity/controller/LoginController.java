...
//        session.setAttribute("kaptcha", text);
        //验证码的归属
        String kaptchaOwner= CommunityUtil.generateUUID();//生成一个随机数，表示是哪个用户的验证码
        ...
        redisTemplate.opsForValue().set(redisKey,text,60, TimeUnit.SECONDS);//设置60秒的过期时间
...
//        String kaptcha = (String) session.getAttribute("kaptcha");
        //从redis里面取验证码，需要先知道key，要想知道key,需要从cookie中取kaptchaOwner
        String  kaptcha=null;
        ...
        // 检查账号,密码
        int expiredSeconds = rememberme ? REMEMBER_EXPIRED_SECONDES:DEFAULT_EXPIERD_SECONDS;
